...
- FreeRTOSを用いたマルチタスク制御により、各機能の並行動作を実現
- スマホからの遠隔操作機能も統合
...
- **12bit ハードウェアDAC（A0ピン）による高音質化**:Arduino UNO R4 WiFi（RA4M1）にのみ搭載されている**本物のアナログ出力（D/Aコンバータ）**ポートをあえてオーディオ出力ピン（A0）に選定。一般的なデジタルピンによるPWM（疑似アナログ）出力とは異なり、キャリアノイズ（高周波スイッチングノイズ）を排出した、クリアで歪みのないノスタルジック・サウンドを生成。
...
https://github.com/user-attachments/assets/7c8e102e-eec4-47ef-b139-debde84b7327
...
- スマホブラウザより http://[ESP32_IP]/ にアクセスし、「FEED ME!」ボタンをタップ。
- ESP32側でリクエストを検知し、即座に液晶(OLED)で「ハートアニメーション」を表示（`EMOTION_FEAST`）。
- 同時にサウンドタスクへイベントが送られ、特別な鳴き声を再生。
...
本プロジェクトをローカル環境でビルド・書き込みするための手順です。

### 1. 依存ライブラリのインストール
以下のライブラリを事前にインストールしてください（PlatformIO環境では `platformio.ini` により自動管理されます）。
- **Adafruit SSD1306** (v2.5.7 以降)
- **Adafruit GFX Library** (v1.11.9 以降)
- **ArduinoJson** (v7.0.0 以降)

### 2. ネットワーク設定の準備
Wi-Fi接続およびNTP時刻同期を行うため、環境に合わせた設定ファイルが必要です。
`src/`（または `include/`）フォルダ内に **`arduino_secrets.h`** という名前のファイルを作成し、以下の内容を記述してください。
...
├── web_page.h          # 遠隔操作用Webインターフェース（HTML/CSS）
├── clock_display.cpp
├── clock_display.h
├── eye_animation.cpp
├── eye_animation.h
...
- **ピンの完全シャットダウン（ハイ・インピーダンス化）**:
 - 無音時にはオーディオピンを単に LOW 出力にするだけでなく、ピンモード自体を強制的に INPUT（入力モード）へ切り替えています。
...
- **デザイン**: `backdrop-filter: blur(10px)` を用いたモダンなUIを実装。ビット調フォント `Press Start 2P` を適用。
- **メモリ効率**: HTML全体を `web_page.h` に分離し、`PROGMEM` / `FPSTR` を使用することで、RAM消費を実質ゼロに抑えながら高機能なWebUIを提供。
- **信頼性**: エサやりリクエスト処理を通信タスクのループ末尾に安全に配置し、非同期イベント駆動を確立。
